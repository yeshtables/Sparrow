...
if __name__ == '__main__':
    ...
    # Set locations
    targetSubKc = args.tempDirName + "/targetsub." + str(os.getpid()) + ".kc"
    ...
    kanProc = subprocess.Popen([args.kanalyze + '/stream', '-k', str(args.kSize), '-f', args.format, '--stdout', '--index'] + args.inFileList, stdout=subprocess.PIPE)
    ...
    for line in kanProc.stdout:
        ...
        if (kanSubProc.returncode != 0):
            err('KAnalyze per-k-mer sub-kmer process died with return code {0}'.format(kanSubProc.returncode), kanSubProc.returncode)
