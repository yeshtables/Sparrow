...
class AnnoGC40to60(AnnoElement):
    def __init__(self):
        AnnoElement.__init__(self, '40 to 60 Test', 0)
...
# Class AnnoGCRev 
class AnnoReverse(AnnoElement):
    def __init__(self):
        AnnoElement.__init__(self, 'Rev GC Content', 0)
    
    def check(self, kmer, pos):
        gcContent = len(re.findall('G|C', kmer)) / len(kmer)
        agcContent = 1-gcContent
        return(agcContent, 0)
...
if (__name__ == '__main__'):
    ...
    if (revCompl):
        ...
        outFile.write('Kmer,Index,Filter')
    
    for anno in annoList:
        outFile.write(',{0}'.format(anno.title))
    
    if (filtScore):
        outFile.write(',Score,No Filt Score\n')
    else:
        outFile.write(',Score\n')
    ...
    for line in inFile:
        ...
        #rev_dict = dict()
        if (revCompl):
            outFile.write('{0},{1},{2},{3},{4}'.format(kmer, revComplStr(kmer), position, 'T' if filterPass == '1' else 'F', rev_comp))
        else:
